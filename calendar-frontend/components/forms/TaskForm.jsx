...
export default function TaskForm({ now, duration, onSave, initialData = null }) {
    ...
    useEffect(() => {
        ...
        const deadline = new Date(now.getTime() + dur * 60 * 1000);
        setForm(p => ({
            ...p,
            date: toDateInputVal(now),
            time: toTimeInputVal(now),
            deadlineDate: toDateInputVal(deadline),
            deadlineTime: toTimeInputVal(deadline),
        }));
    ...
    const handleSave = () => {
        ...
        onSave?.({ type: 'task', ...form, file: selectedFile });
    };
        ...
        <div className="space-y-4 py-2">
            <FieldRow icon={CheckSquare}>
                <div className="flex-1 min-w-0">
                    ...
                    {isTitleEmpty && (
                        <p className="text-[10px] text-red-500 mt-1 ml-1 animate-pulse">Tên việc không được để trống</p>
                    )}
                </div>
            </FieldRow>
