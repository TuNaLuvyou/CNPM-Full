        ...
        <FieldRow icon={Users}>
            <div className="flex flex-col w-full gap-2">
                <div className="flex items-center justify-between px-1">
                    <span className="text-sm font-semibold text-slate-700 dark:text-white">{t('contacts_panel.guests', lang)}</span>
                    {isOwner && (
                        <button 
                            onClick={(e) => {
                                e.preventDefault();
                            ...
                            className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 px-3 py-1.5 rounded-full transition border border-transparent hover:border-blue-200 dark:hover:border-blue-500/30 flex items-center gap-1.5"
                        >
                            {showGuestPicker ? (
                ...
                <div className="bg-slate-50/50 dark:bg-[#1f1f1f] border border-slate-200 dark:border-[#484848]/60 rounded-2xl p-1.5 min-h-[50px] flex flex-col gap-1.5 transition-all">
                            ...
                            <div className="p-2 border-b border-slate-100 dark:border-[#484848] bg-slate-50/50 dark:bg-[#1f1f1f]">
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-8 pr-3 py-1.5 bg-white dark:bg-[#1f1f1f] border border-slate-200 dark:border-[#484848] rounded-lg text-xs outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium"
                                    />
                                </div>
                            </div>
                                        ...
                                        return (
                                            <div key={f.id} 
                                                ...
                                                className={`w-full flex items-center gap-3 p-2 rounded-lg transition-all group mb-0.5 cursor-pointer
                                                    ${isAdded ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50 dark:hover:bg-[#2d2d2d] text-slate-600 dark:text-white active:scale-[0.98]'}`}>
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-transform group-hover:scale-110
                                                    ${isAdded ? 'bg-blue-600 text-white' : 'bg-white dark:bg-[#1f1f1f] border border-slate-200 dark:border-[#484848] text-blue-600 shadow-sm'}`}>
                                                    {initial}
                                                </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-bold text-slate-700 dark:text-white tracking-tight">{g.invitee_details?.name || g.invitee_details?.username}</span>
